...
,	m_pPlugin( NULL )
{
	m_pPlugin = CPRXManager::FindPlugin( m_szFirstFile );
...
/*	if ( m_pPlugin != NULL )
	{
		p_item = new CFileOptionsMenuItem( OPTION_EXECUTE, m_pPlugin->GetMenuText() );
		p_item->SetCallback( OptionSelectedCallback );
		m_pOptions->AddItem( p_item );
	}*/
...
/*	if ( m_pPlugin != NULL )
	{
		...
		case CPRXPlugin::MUSIC_PLUGIN:
			{
				static_cast< const CMusicPlugin * >( m_pPlugin )->Play( CFileHandler::GetFile() );
			}
			break;
		}
	}*/
...
void	CFileOptions::CopyCallback()
{
	...
	if ( box.Show() == CModalMessageBox::EXIT_CROSS )
	{
		...
		if ( success == false )
		{
			CErrorMessage( "Error copying file(s)!" );
		}
		else
		{
...
void	CFileOptions::DeleteCallback()
{
	...
	if ( box.Show() == CModalMessageBox::EXIT_CROSS )
	{
		...
		if ( success == false )
		{
			CErrorMessage( "Error deleting file(s)!" );
		}
		else
		{
...
void	CFileOptions::MoveCallback()
{
	...
	if ( box.Show() == CModalMessageBox::EXIT_CROSS )
	{
		...
		if ( success == false )
		{
			CErrorMessage( "Error moving file(s)!" );
		}
		else
		{
...
void	CFileOptions::RenameCallback()
{
	...
	if ( confirmed == true )
	{
		...
		if ( return_code == CTextInput::RC_OK )
		{
			if ( success == false )
			{
				CErrorMessage( "Error renaming file(s)!" );
			}
			else
			{
