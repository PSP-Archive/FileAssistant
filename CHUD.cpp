...
//   Local Constants
//**********************************************************************************
static const CString	VERSION_STRING( "v1.0" );
...
static const CString	s_szDays[ 7 ] =
{
	"Mod",	"Tue",	"Wed",	"Thu",	"Fri",	"Sat",	"Sun"
};

static const CString	s_szMonths[ 12 ] =
{
...
void	CHUD::RenderInternal()
{
	...
	if ( CMusicFileHandler::IsPlaying() == true )
	{
		...
		const CString	info( CMusicFileHandler::GetInfo() );

		const V2		play_text_size( CFont::GetDefaultFont()->GetStringSize( play_time ) );
		const V2		info_text_size( CFont::GetDefaultFont()->GetStringSize( info ) );
		const V2		file_text_size( CFont::GetDefaultFont()->GetStringSize( filename ) + V2( 32.f, 0.f ) );
		const V2		music_pos( CSkinManager::GetV2( "hud", "song_title_pos", V2( 140.f, 244.f ) ) );
		const V2		music_size( CSkinManager::GetV2( "hud", "song_title_size", V2( 200.f, 12.f ) ) );

		// display time and filename
		CFont::GetDefaultFont()->Print( play_time, music_pos - V2( 0.f, play_text_size.y ), text_color );
		//CFont::GetDefaultFont()->Print( filename, music_pos - V2( -(play_text_size.x + 5), play_text_size.y ), text_color );

		CGfx::DrawQuad( music_pos, music_size, 0x40000000 );

		if ( info_text_size.x > music_size.x )
		{
			CGfx::SetClipRegion( music_pos, music_size );

			CFont::GetDefaultFont()->Print( info, music_pos + V2( s_MusicOffset, 0.f ), text_color );
			CFont::GetDefaultFont()->Print( info, music_pos + V2( s_MusicOffset + info_text_size.x, 0.f ), text_color );

			CGfx::DisableClipRegions();

			s_MusicOffset -= MUSIC_SCROLL_SPEED * CFrameWork::GetElapsedTime();

			if ( s_MusicOffset < -info_text_size.x )
			{
				s_MusicOffset += info_text_size.x;
			}
		}
		else
		{
			CFont::GetDefaultFont()->Print( info, music_pos, text_color );
		}
	}
	else
	{
