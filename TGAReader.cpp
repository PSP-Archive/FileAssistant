...
static void	MergeBytes( u8 * const pixel, const u8 * const p, int bytes )
{
	...
	{
		TGA_ASSERT( 0, "Unknown number of bytes!" );
	}
}
...
CTexture *	CTextureManager::ReadTGA( CFile * const p_file )
{
	...
	{
		TGA_ASSERT( 0, "Can only handle image type 2 and 10\n" );
	...
	{
		TGA_ASSERT( 0, "Can only handle pixel depths of 16, 24, and 32\n" );
	...
	{
		TGA_ASSERT( 0, "Can only handle colour map types of 0 and 1\n" );
	...
	TGA_ASSERT( p_texture != NULL, "Out of memory!" );
		...
		else
		{
			...
			while ( y >= 0 )
			{
				...
				if ( header.datatypecode == 2 )
				{
					...
					{
						TGA_ASSERT( 0, "Unexpected end of file\n" );
